...
public:
    GraphWidget(QVector<Point>& points, QVector<Edge>& edges, QWidget* parent = nullptr)
        ...
        setWindowTitle(tr("Коммивояжер"));

        // Создание поля для вывода текста
                textOutput = new QTextEdit(this);
                textOutput->setReadOnly(true);
                textOutput->setGeometry(10, 40, 385, 26);

        solveTSP();

        textOutput->append("Минимальная дистанция: " + QString::number(minDistance));

        QPushButton* addPointButton = new QPushButton("Добавить точку", this);
        addPointButton->move(10, 10);
        connect(addPointButton, &QPushButton::clicked, this, &GraphWidget::addPoint);

        QPushButton* removePointButton = new QPushButton("Удалить точку", this);
        removePointButton->move(110, 10);
    ...
    void solveTSP() {
        ...
        do {
            ...
            for (int i = 0; i < numPoints - 1; ++i) {
                ...
                auto edgeIt = find_if(edges.begin(), edges.end(), [&](const Edge& edge) {
        ...
        cout << "Расстояние: " << minDistance << endl;
    ...
    void drawOptimalPath() {
        ...
        // Подключение первой и последней точки
    ...
    void addPoint() {
        bool ok;
        double x = QInputDialog::getDouble(this, "Добавить точку", "Введите Х координату:", 0, -1000, 1000, 1, &ok);
        if (!ok)
            return;
        double y = QInputDialog::getDouble(this, "Добавить точку", "Введите Y координату:", 0, -1000, 1000, 1, &ok);
        if (!ok)
            return;
    ...
    void removePoint() {
        if (points.isEmpty()) {
            QMessageBox::information(this, "Удалить точку", "Нет точек для удаления");
            return;
        }

        bool ok;
        int index = QInputDialog::getInt(this, "Удалить точку", "Введите номер точки:", 1, 1, points.size(), 1, &ok);
        if (!ok)
            return;
    ...
    void addEdge() {
        if (points.size() < 2) {
            QMessageBox::information(this, "Добавить ребро", "Для добавления ребра требуется не менее двух точек");
            return;
        }

        bool ok;
        int sourceIndex = QInputDialog::getInt(this, "Добавить ребро", "Введите номер исходной точки:", 1, 1, points.size(), 1, &ok);
        if (!ok)
            return;
        int destinationIndex = QInputDialog::getInt(this, "Добавить ребро", "Введите номер конечной точки:", 1, 1, points.size(), 1, &ok);
        if (!ok)
            return;
        double weight = QInputDialog::getDouble(this, "Добавить ребро", "Введите вес ребра:", 0, 0, 1000, 1, &ok);
        if (!ok)
            return;
    ...
    void removeEdge() {
        if (edges.isEmpty()) {
            QMessageBox::information(this, "Удалить ребро", "Нет ребер для удаления.");
            return;
        }

        bool ok;
        int sourceIndex = QInputDialog::getInt(this, "Удалить ребро", "Введите номер исходной точки:", 1, 1, points.size(), 1, &ok);
        if (!ok)
            return;
        int destinationIndex = QInputDialog::getInt(this, "Удалить ребро", "Введите номер конечной точки:", 1, 1, points.size(), 1, &ok);
        if (!ok)
            return;
        ...
        if (it != edges.end()) {
            ...
            QMessageBox::information(this, "Удалить ребро", "Ребро не найдено");
        }
    }
...
private:
    ...
    double minDistance = numeric_limits<double>::infinity(); // Измененное объявление переменной
    ...
    void updateMinDistance() {
        ...
        solveTSP();
        textOutput->clear();
        textOutput->append("Расстояние: " + QString::number(minDistance));
    }
};
